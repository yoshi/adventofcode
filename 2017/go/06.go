...
func main() {
	...
	f, err := os.Open( "06.data")
	//f, err := os.Open( "06.test")
	defer f.Close( )
		...
		for scanner.Scan() {
			...
			for ! iloop {
				...
				for i,h := range history {
					if eq( h, banks ) {
						// match found
						fmt.Printf( "uh i is %d and so the answer is %d?\n", i, round - i )
						iloop = true
					}
				}

				if iloop {
					fmt.Printf( "Loop Detected on iteration %d\n", round )
				} else {
					round++
...
func spread(b []int) []int {
	maxv := -1
	maxix := -1
	nb := make([]int, len(b), cap(b))
	copy( nb, b )

	for i := len(b) - 1; i >= 0; i-- {
		if b[i] >= maxv {
			//fmt.Printf( "%v max detected at %d: %d\n", b, i, b[i] )
			maxv = b[i]
			maxix = i
		}
	}
	...
	for dist > 0 {
		...
		ix = int(math.Mod(float64(ix + 1), float64(len(nb))))
		nb[ix]++
		dist--
	}
